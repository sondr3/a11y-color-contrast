 ...
 */
export function wcag(foreground: Color, background: Color = [255, 255, 255], options?: WCAG): WCAGScore {
	const score = contrast(foreground, background)
	const pass = score >= contrastLevel(setWCAG({ ...options }))
 ...
 * See https://www.w3.org/WAI/WCAG22/Techniques/general/G17.html for concrete details for WCAG 2.2.
 */
export function wcagIsReadable(foreground: Color, background: Color = [255, 255, 255], options?: WCAG): boolean {
	const score = contrast(foreground, background)
	return score >= contrastLevel(setWCAG({ ...options }))
}
